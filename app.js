class MNISTApp {
    constructor() {
        ...
        // Сразу устанавливаем CPU до инициализации всего остального
        this.forceCPU();
    }

    async forceCPU() {
        try {
            // Устанавливаем CPU бэкенд
            await tf.setBackend('cpu');
            console.log('CPU backend forced');
            
            // Проверяем, что действительно CPU
            const backend = tf.getBackend();
            console.log('Actual backend:', backend);
            
            this.showStatus(`Using ${backend} backend`);
            
            // После установки бэкенда инициализируем UI
            this.initializeUI();
            
        } catch (error) {
            console.error('Failed to set CPU backend:', error);
            this.showStatus('WARNING: Using default backend');
            this.initializeUI();
        }
    }
    ...
    async onLoadData() {
        try {
            ...
            const trainMin = await trainData.xs.min().data();
            const trainMax = await trainData.xs.max().data();
            const testMin = await testData.xs.min().data();
            const testMax = await testData.xs.max().data();
            
            this.showStatus(`Train data range: [${trainMin[0].toFixed(3)}, ${trainMax[0].toFixed(3)}]`);
            this.showStatus(`Test data range: [${testMin[0].toFixed(3)}, ${testMax[0].toFixed(3)}]`);
    ...
    async onTrain() {
        ...
        try {
            ...
            // Используем меньше данных для стабильности
            ...
            // Уменьшаем batch size для CPU
            await this.model.fit(trainXs, trainYs, {
                epochs: 5,
                batchSize: 32, // Уменьшили с 128 до 32
    ...
    createClassifier() {
        ...
        // Упрощенная архитектура для CPU
        model.add(tf.layers.conv2d({
            filters: 16, // Уменьшили с 32 до 16
            kernelSize: 3,
            activation: 'relu',
            inputShape: [28, 28, 1],
        ...
        model.add(tf.layers.conv2d({
            filters: 32, // Уменьшили с 64 до 32
            kernelSize: 3,
            activation: 'relu',
            kernelInitializer: 'heNormal'
        }));
        ...
        model.add(tf.layers.dense({
            units: 64, // Уменьшили с 128 до 64
            activation: 'relu',
            kernelInitializer: 'heNormal'
        }));
    ...
    async onTestFive() {
        ...
        try {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                label.textContent = `Pred: ${predArray[i]} | True: ${trueArray[i]}`;
