class MNISTApp {
    ...
    async onTestDenoise() {
        ...
        try {
            ...
            // Создаем две модели с разным pooling
            const maxPoolModel = this.createDenoiserWithPooling('max');
            const avgPoolModel = this.createDenoiserWithPooling('avg');
            
            // Копируем веса из обученной модели (если есть)
            if (this.denoiserModel) {
                const weights = this.denoiserModel.getWeights();
                maxPoolModel.setWeights(weights);
                avgPoolModel.setWeights(weights);
            }
            ...
            const container = document.getElementById('previewContainer');
            container.innerHTML = '<h3>Denoising Comparison: Max Pooling vs Average Pooling</h3>';
            ...
            const headers = ['Noisy', 'Max Pooling', 'Average Pooling', 'Original'];
            ...
            for (let i = 0; i < 5; i++) {
                ...
                tf.dispose([noisyTensor, maxTensor, avgTensor, originalTensor]);
            }
            
            tf.dispose([original, noise, noisy, noisyFlat, denoisedMaxFlat, denoisedAvgFlat, denoisedMax, denoisedAvg]);
    ...
    createDenoiserWithPooling(poolType) {
        ...
        if (poolType === 'max') {
            model.add(tf.layers.maxPooling2d({ poolSize: 2, inputShape: [28, 28, 1] }));
        } else {
            model.add(tf.layers.averagePooling2d({ poolSize: 2, inputShape: [28, 28, 1] }));
        }
